...
class ServiceDetail(models.Model):
    ...
    image_main = models.FileField('Фото на Главной 600*500', upload_to='services', blank=True)
    text_1 = models.CharField('Текст на Главной', max_length=5000, blank=True)
    title_2 = models.CharField('Название текста в разделе', max_length=50, blank=True)
    text_2 = models.CharField('Текст в разделе', max_length=5000, blank=True)
    ...
    text_1_trim.short_description = "Текст на Главной"

    def text_2_trim(self):
        return self.text_2[:100]
    text_2_trim.short_description = "Текст в разделе"
    ...
    def image_main_preview(self):
        if self.image_main:
            return mark_safe('<img src="%s" style="width:100px; height:70px;" />' % self.image_main.url)
        else:
            return 'No Image Found'

    image_main_preview.short_description = 'Фото на Главной'
